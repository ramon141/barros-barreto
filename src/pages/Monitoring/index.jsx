import { Grid, TextField, CircularProgress, FormControl, InputLabel, Typography, Card, CardContent, Select, MenuItem, IconButton, Button } from '@mui/material';
...
export default function Monitoring() {
  ...
  const [isLoadingMensureInterval, setIsLoadingMensureInterval] = useState(false);
  ...
  const updateMensureInterval = () => {
    ...
    api.patch(`patients/${patientId}`, data)
      .then((response) => {
        setPatient(prev => { return { ...prev, mensureInterval } })
      })
      .catch((err) => {
  ...
  const onDischarge = (dischargedFromHospital) => {
    ...
    api.patch(`patients/${patientId}`, data)
      .then((response) => {
        setPatient(prev => { return { ...prev, dischargedFromHospital } })
  ...
  return (
    <Card style={{ margin: 20 }}>
      <CardContent>
          ...
          <Grid item container xs={12} sm={4} md={4} lg={4}>
            ...
            <Grid item xs={2} sm={2} md={2} lg={2}>
              <IconButton
                style={{
                  width: '100%',
                  color: mensureInterval === patient.mensureInterval ? '#1B98E0' : '#ff8833'
                }}
        ...
        <ModalChoiceReportFormat onChoice={onChoiceFormat} open={isOpenModalChoiceFormat} setOpen={setIsOpenModalChoiceFormat} />

        <ModalDischarge onChoice={onDischarge} open={isOpenModalDischarge} setOpen={setIsOpenModalDischarge} />
